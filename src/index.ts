...
async function main() {
  ...
  const channels = await connectChannels(discordClient, [
    devDiscordChannel,
    redditHiringDiscordChannel,
  ]);
  console.log(1, channels);
  const forHireWatcher = new RedditWatcher(
    snoowrapClient,
    [consoleChannel, ...channels],
    "forhire"
  );
  forHireWatcher.postFilter = (notice: Notice): boolean => {
    return notice.title.toLowerCase().includes("[hiring]");
  };
  forHireWatcher.commentFilter = (notice: Notice): boolean => {
    return false;
  };
  const remoteJsWatcher = new RedditWatcher(
    snoowrapClient,
    [consoleChannel, ...channels],
    "remotejs"
  );
  ...
  remoteJsWatcher.commentFilter = (notice: Notice): boolean => {
    return false;
  };
